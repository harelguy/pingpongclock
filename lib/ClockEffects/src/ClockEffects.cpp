...
ClockEffects::ClockEffects( int num_leds) {
		NUM_LEDS = num_leds;
	}
	
	void rainbow_beat(CRGB* leds) {
		uint8_t beatA = beatsin8(17, 0, 255);                        // Starting hue
		uint8_t beatB = beatsin8(13, 0, 255);
		fill_rainbow(leds, NUM_LEDS, (beatA + beatB) / 2, 8);            // Use FastLED's fill_rainbow routine.
	}

	void displayPlasma(CRGB* leds) {
		if (!plasmaInit) {
	...
	void plasma(CRGB* leds) {                                                 // This is the heart of this program. Sure is short. . . and fast.
